...
class ResultLineParser {
	...
	private void parseData(byte[] line){
		ArrayList<byte[]> split=Tools.split(line, 0, (byte)'\t');
		qTaxID=Tools.parseInt(split.get(qTaxIDColumn), 0);
		rTaxID=Tools.parseInt(split.get(rTaxIDColumn), 0);
		qBases=Tools.parseLong(split.get(qBasesColumn), 0);
		rBases=Tools.parseLong(split.get(rBasesColumn), 0);
		qSize=Tools.parseLong(split.get(qSizeColumn), 0);
		rSize=Tools.parseLong(split.get(rSizeColumn), 0);
		ani=Tools.parseDouble(split.get(aniColumn), 0);
		byte[] ssuArray=split.get(ssuColumn);
		ssu=ssuArray.length==1 && ssuArray[0]=='.' ? -1 : Tools.parseDouble(ssuArray, 0);
	...
	private TaxNode getTaxNode(String fname){
		...
		if(name.startsWith("tid_")){
			...
			int x=Tools.parseInt(fname, 4, idx2);
